...
/**
 * Container menu for Ancient Debris Extractor GUI
 */
public class AncientDebrisExtractorMenu extends AbstractContainerMenu {
    private final AncientDebrisExtractorBlockEntity blockEntity;
    private final Level level;
    private final ContainerData data;

    // Client constructor
    public AncientDebrisExtractorMenu(int containerId, Inventory playerInventory, FriendlyByteBuf extraData) {
        this(containerId, playerInventory, playerInventory.player.level().getBlockEntity(extraData.readBlockPos()),
                new SimpleContainerData(3));
    }

    // Server constructor
    public AncientDebrisExtractorMenu(int containerId, Inventory playerInventory, BlockEntity blockEntity, ContainerData data) {
        super(ModMenuTypes.ANCIENT_DEBRIS_EXTRACTOR.get(), containerId);
        checkContainerSize(playerInventory, AncientDebrisExtractorBlockEntity.TOTAL_SLOTS);
        this.blockEntity = (AncientDebrisExtractorBlockEntity) blockEntity;
        ...
        // Input slot (nether blocks) - position 56, 17
        this.addSlot(new SlotItemHandler(this.blockEntity.getItemHandler(), AncientDebrisExtractorBlockEntity.INPUT_SLOT, 56, 17) {
            @Override
        ...
        this.addSlot(new SlotItemHandler(this.blockEntity.getItemHandler(), AncientDebrisExtractorBlockEntity.FUEL_SLOT, 56, 53) {
            @Override
            public boolean mayPlace(@NotNull ItemStack stack) {
        ...
        // Output slots (3 slots) - positions starting at 116, 17
        for (int i = 0; i < AncientDebrisExtractorBlockEntity.OUTPUT_SLOT_COUNT; i++) {
            int slotIndex = AncientDebrisExtractorBlockEntity.OUTPUT_SLOT_START + i;
            int x = 116;
            int y = 17 + (i * 18);
    ...
    public @NotNull ItemStack quickMoveStack(@NotNull Player player, int index) {
        ...
        if (slot.hasItem()) {
            ...
            // Slots: 0-35 = player inventory, 36 = input, 37 = fuel, 38-40 = output
            ...
            if (index >= playerInventoryEnd) {
                ...
                if (isValidNetherBlock(originalStack)) {
    ...
    public boolean stillValid(@NotNull Player player) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                player, ModBlocks.ANCIENT_DEBRIS_EXTRACTOR.get());
    }
